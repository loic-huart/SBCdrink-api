...
import Service, { IService } from '../../shared/service'
import { IModelIngredient } from '../../models/types'

interface IIngredientService extends IService<IIngredient> {
  // getInstance: () => IIngredientService
...
class IngredientService extends Service<IIngredient> implements IIngredientService {
  // private static instance: IIngredientService
  ...
  public static getInstance (): IngredientService {
    if (IngredientService.instance === undefined) {
      ...
      IngredientService.setModel(Ingredient)
    }

    return IngredientService.instance
  }

  // public async find ({ sort }: IPayloadFindIngredients): Promise<{ datas: IIngredient[] }> {
  //   const ingredients = await Ingredient.find().sort({ updated_at: sort === 'desc' ? -1 : 1 })
  //   return {
  //     datas: serializeIngredients(ingredients)
  //   }
  // }
